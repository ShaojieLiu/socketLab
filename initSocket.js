    ...
    io.on('connection', function(socket){
        ...
        socket.on('disconnect', function(){
            ...
            if(onlineUsers.hasOwnProperty(socket.name)) {
                ...
                var obj = {userId:socket.name, userName:user.userName, roomId: user.roomId};
        ...
        socket.on('operation', function(obj){
            ...
            const userInRoom = _.map(onlineUsers, (user, id) => user).filter(user => user.roomId === roomId);
            userInRoom.forEach(user => io.to(user.socketId).emit('operation', obj))

            // io.emit('operation', obj);
            console.log(roomId+'房间的'+socket.userName+'做了'+obj.action.type);
        });
